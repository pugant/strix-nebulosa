...
- `2026-09-09` **Wave-5 restore-CRC campaign (roofline-first)** — three wall-critical probes decided the cards before any implementation (~7 GPU-min vs 35 planned). The one GO is host-side: the `--cache-disk-persist` restore CRC32 verify moves to a bit-exact PCLMUL fold (**12.9×**, 0.63 → 8.07 GB/s, runtime dispatch, table fallback verbatim) — the canonical restore entry is 2,272,282,732 B and CRC is **69.8% of the real restore wall** (the historical 1.57 s was `load_ms` without CRC; expected restore −64%), deployed as `qwen4exp-mtp-vk-optim-w5` with per-probe battery shas bit-identical 12/12 and no new flags. The FA prefill tile card is a **structural NO-GO**: coopmat2 is never active on gfx1151/RADV (every FA call runs COOPMAT1 with Br16/Bc64 hardcoded), and the correct Br32 dual-panel halves KV rereads but the LDS cost drops occupancy 2→1 WG/CU and loses on **every** FA shape (c32k −5.96%) — the zero-reuse roofline was refuted by measurement. Host sampling closed in the opening probe at 0.32% of the round wall; note: [2026-09-09-vulkan-optimization-wave5.md](2026-09-09-vulkan-optimization-wave5.md).
...
**What shipped:** the production serving config (Vulkan/RADV + `--no-mmap` + KV q8_0, cache-ram sized for the agent), and the closed levers (KV-quant, checkpoint/ring, hipCUB) documented so they stay closed. The optimization-campaign speedups (graph reuse + dense decode, waves 2-5 incl. the split-k default of `optim-w4` and the PCLMUL restore-CRC of `optim-w5`) are documented under [PATCHES.md](../../PATCHES.md) and the wave-4/wave-5 notes above.
...
- `2026-09-10` **Wave-6: the round engine** — keep the accepted MTP boundary through the partial-reject reset (+5.4% t/s, bit-identical battery), requant only the drafter head, stop paying the conv-ring conts, read the persist entry once, fold the save CRC in-stream, cache the conversation prefix and the template analysis (+8.7% end-to-end). Note: [2026-09-10-vulkan-optimization-wave6.md](2026-09-10-vulkan-optimization-wave6.md).
- `2026-09-11` **Wave-7: FR-Spec and the router** — the draft-vocabulary trim (FR-Spec/d2t) measured honestly into a **NO-GO**: lossless by construction and by gate, but at 65k rows the acceptance loss on thinking-heavy workloads costs more than the −50% head bytes save, and at 128k the trimmed Q8 head is byte-equal to the production Q4 head — no margin; the lossless port stays in the engine as a no-op. The lever that did move is the MoE router requant (48 `ffn_gate_inp` F32 → ROCmFP4, 7.03× the router-path bytes per forward, ppl and battery clean, c32k +4.24%), plus an allocation-free BPE session (tokenize 1.80×, per-request path ~20 ms with the jinja render as the structural residual), the k-slot graph cache landed opt-in after its rev-2 fix, and two bit-exact host-side fusions now active on Vulkan. +3.9% end-to-end (battery bit-identical), deployed below the pre-declared +5% gate on an explicit GO with the FR-Spec lever out. Note: [2026-09-11-vulkan-optimization-wave7.md](2026-09-11-vulkan-optimization-wave7.md).
